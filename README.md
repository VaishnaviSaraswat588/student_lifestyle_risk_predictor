...
Approach
Feature engineering: one-hot encode department and gender, add the High_Stress threshold flag described above.
Model: logistic regression (scikit-learn), all features standardized, class_weight="balanced" — a deliberate choice, since depression is only ~10% of rows and a health-adjacent risk tool should prioritize catching real cases (recall) over raw accuracy.
Split: 80/20 train/test, stratified on the target, random_state=42.
Export: the fitted coefficients, intercept, and standardization parameters are copied directly into index.html as a JS object, so the exact same linear model runs client-side with zero inference cost and zero data leaving the browser.
...
CGPA (strongest — lower CGPA → higher predicted risk)
Crossing the stress-8+ threshold
Continuous stress level
Sleep duration (more sleep → lower risk)
Age, department, gender, study hours, social media, and physical activity all contribute far less.
Honest limitations
